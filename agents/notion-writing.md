...
| N5 | parent 種別の取り違え | `notion-create-pages` が意図しない場所に作られる／エラー | DB に行を作るのに `page_id` を、ページの子ページを作るのに `data_source_id` を渡す等 | **DB に行を作るなら parent=`data_source_id`**、**ページの子ページなら parent=`page_id`**。例：スカウト評価ログ DB＝data source `437a2bad-94d6-4397-8dc4-1aa986a34c3d` | §3 |
...
| `tools/scout_eval.py` | スカウト評価ログ DB へ create／backfill（`create --from-block "…"` で「登録ブロック（パイプ1行）」をそのまま取り込み。`backfill --media-id … --reply-date … --result …` で結果書き戻し） |
...
- **DB に1行（レコード）を作る** → parent＝`data_source_id`（例：スカウト評価ログ DB＝`437a2bad-94d6-4397-8dc4-1aa986a34c3d`、スカウト DB＝`2597d017-b6a0-801b-8185-000ba4b9661e`）。プロパティを指定して作る。
