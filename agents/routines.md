...
4. 各候補者について agents/candidate-profile.md「初回生成」(セクション5.2)に従い、Notion 候補者/パイプライン/
   ...
   **【最重要・1名ずつ完結】候補者ごとに「作成（または §2 で既存 doc を発見）→ ただちに `salesforce_dml_records` で `Contact.Craft_Profile_URL_SY__c` に `craftdocs://open?blockId=<rootBlockId>` を書き戻す」までを終えてから、次の候補者へ進む。末尾に一括で書き戻さない（3:30 締切・予算切れ・中断で必ず書き戻し漏れが出る）。この書き戻しは判定0（決定論的 dedup）の生命線で、抜けると同じ候補者を毎晩再評価し続ける。公開共有リンクは保存しない。**

## 共通ルール
- **書き戻し（`Account.Craft_Profile_URL_SY__c` / `Contact.Craft_Profile_URL_SY__c`）は対象1件ごとに即時実行し、末尾に一括しない。中断・予算切れがあっても「作成（または既存 doc 発見）→ 書き戻し」を1件単位で完結させてから次へ。書き戻し漏れは判定0の冪等性を壊し、翌晩以降の再評価ループを生む。**
...
4. 各候補者について agents/candidate-profile.md「初回生成」(5.2)に従い Craft に「{漢字氏名}（{ふりがな}）」で作成（**当て字禁止**）。
   ...
   **【最重要・1名ずつ完結】候補者ごとに「作成（または既存 doc 発見）→ ただちに `salesforce_dml_records` で `Contact.Craft_Profile_URL_SY__c` に `craftdocs://open?blockId=<rootBlockId>` を書き戻す」までを終えてから次へ進む。末尾一括にしない（中断で書き戻し漏れが出る）。判定0の冪等性の生命線。公開共有リンクは保存しない。**
