...
- **モデル：** Sonnet 4.6
- **リポジトリ：** `pnl-sato/pnl`
- **コネクタ：** Salesforce / Notion / Craft / Gmail / Google Drive / Slack（不要分は外す）
- **環境：** Default（Trusted）。企業 Web への WebFetch が 403 になり得るが内部ソース中心の土台作成には支障なし。
...
2. Craft フォルダ「12_Client｜企業」(folder ID: F41A3C28-9B9B-47C7-BAC1-0D8431AF73A9)を search し、
   既に {社名}/{社名}.md がある社を把握する。
3. 「SF登録あり かつ Craft未作成」を抽出し、最大3社を選ぶ
...
## Part 2：候補者プロファイル backfill（選考中アクティブ優先・最大5名）
1. Notion パイプライン DB(collection://20f7d017-b6a0-807c-a60f-000b827c6841)から、選考中=アクティブ
   （クローズ/見送り/決定済を除く進行中）の候補者を一覧化。
2. Craft フォルダ「13_Candidate｜候補者」(folder ID: 05BC363C-0FC2-4B15-AB3D-7C335AA5AB4E)を search し、
   既にプロファイル md がある候補者を把握する。
3. 「アクティブ選考中 かつ Craft未作成」を抽出し、最大5名を選ぶ（優先順：選考フェーズが進んでいる順 > 直近更新順）。
...
1. Salesforce で取引先(Account)=クライアント企業を一覧化（取引終了・クローズ系は除外）。
2. Craft「12_Client｜企業」(folder ID: F41A3C28-9B9B-47C7-BAC1-0D8431AF73A9)を search し、既存 {社名}/{社名}.md を把握。
3. 「SF登録あり かつ Craft未作成」を抽出し、最大10社（優先順：注力 > 関わり度合い高 > 直近動き > 社名昇順）。
...
## Part 2：候補者プロファイル backfill（最大20名）
1. Notion パイプライン DB(collection://20f7d017-b6a0-807c-a60f-000b827c6841)で、選考中=アクティブを優先一覧化。
2. Craft「13_Candidate｜候補者」(folder ID: 05BC363C-0FC2-4B15-AB3D-7C335AA5AB4E)を search し、既存 md を把握。
3. 「アクティブ かつ Craft未作成」を優先抽出。アクティブが尽きたら、Notion 候補者 DB
   (collection://2057d017-b6a0-80fc-9e8d-000b3b6ab37e)の Craft未作成候補者へ広げてよい。合計最大20名。
...
4. **Environment = Default**（Web 補完を強化したい場合のみ Custom/Full に上げる）。
5. **Connectors**：Salesforce / Notion / Craft / Gmail / Google Drive / Slack を残し、他は外す。
