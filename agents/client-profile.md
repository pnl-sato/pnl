...
ポジションの正本は **SF Opportunity**。`LimitedreleaseURL__c` の有無で2カテゴリに分類してマッチング評価する：
...
2. **SF Opportunity を query**（`StageName != クローズ系`、Limit 100程度）→ ポジション universe 確定
3. 各 Opportunity を 2 カテゴリに分類：
   - 🟢 = `LimitedreleaseURL__c IS NOT NULL`
   - 🟡 = `LimitedreleaseURL__c IS NULL`（公開のみ）
4. 注力 YES（Notion ポジション DB の `注力` フィールド）に該当するものは Craft 注力ポジション md でさらに深掘り
5. Email 共有由来を context として fetch：
   - 該当クライアントの `label:SY/{社コード}` 直近メール
   - `label:SY/ATS-*` の該当候補者氏名を含むスレッド
6. 各ポジションについて、以下5軸で構造化評価：
...
7. **🟢 と 🟡 を別セクションでランキング**して上位3〜5件ずつ提案：

```
## 🟢 即推薦可（N件）
1. {クライアント} / {ポジション} — 適合 X.X
   - 推薦時に強調すべき点
   - 懸念点／事前に詰めるべきこと
   - 推薦の進め方（具体的なアクションリスト）

## 🟡 新規打診候補（N件）— P&L 未契約 or 未登録
1. {クライアント} / {ポジション} — 適合 X.X / 一般公開URL: {url}
   - 提案：人事窓口にコンタクト推奨
...
1. 該当ポジション md（Craft）または SF Opportunity（注力 NO の場合）を fetch
