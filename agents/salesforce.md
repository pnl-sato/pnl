...
- `ContentNote`（打ち合わせ議事録・メモ）
- `Employment_History__c`（在籍履歴。必要時に都度 describe して使う）
- ※ `slackv2__*` 等は連携・設定用で業務データではない。

## ⓪ ContentNote（打ち合わせ議事録・メモ）— 打ち合わせ記録の第一参照先

**打ち合わせ記録・議事録を探すときは、まず ContentNote を確認する。** Task/Event ではなく ContentNote にメンバーが議事録を残している。

- 検索：`salesforce_search_all` で `searchTerm` にクライアント名等を渡し、`ContentNote` を対象に含める
- 既定取得項目：`Id, Title, TextPreview, CreatedDate, LastModifiedDate, CreatedBy.Name`
- **全文取得**：`TextPreview` は255文字で切れる。全文が必要な場合は `LatestPublishedVersionId` → `ContentVersion.VersionData` を Anonymous Apex（`cv.VersionData.toString()`）で取得する（Content/VersionData は base64 blob 型のため通常の SOQL では読めない）
- ContentNote は Account に紐づいていることが多い。`ContentDocumentLink` で親レコードとの関連を辿れる
