...
| `imessage_sync.py` | 候補者との iMessage/SMS だけを chat.db（読取専用）から抜き Notion に保存。番号マスターで許可リスト・差分同期・常駐可 | `python3 tools/mac/imessage_sync.py [--dry-run\|--probe\|--setup]` | 標準ライブラリのみ（要 FDA） |
