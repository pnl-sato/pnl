...
| `line_import.py` | LINE の「トーク履歴を保存」.txt を解析し、iMessage と同じ候補者ページへ `[LINE]` タグで追記（半自動）。重複は署名で防止 | `python3 tools/mac/line_import.py <export.txt> [--candidate "氏名"\|--candidate-id <id>] [--self "氏名"] [--dry-run]` | 標準ライブラリのみ |
...
### LINE の取り込み（`line_import.py`・半自動）

LINE はローカル DB が暗号化（Letter Sealing / E2EE）され、個人アカウントの履歴を引き出す API も
無いため、iMessage のような全自動はできない。唯一の正攻法は LINE（Mac/Win/スマホ）の
**「トーク履歴を保存／送信」で `.txt` を書き出す** こと。それを `line_import.py` に渡すと、解析して
**iMessage と同じ候補者の履歴ページ本文へ `[LINE]` タグ付きで時系列追記** する（保存形式・送受信の
色分けは共通）。これで Claude Code は候補者1ページで iMessage / SMS / LINE を横断して網羅読みできる。

```bash
# 1) LINE で対象トークを開き「トーク履歴を保存」→ export.txt（古い分も欲しければ保存前に上へスクロール）
# 2) 下見（Notion 書込なし）。相手名はヘッダから自動抽出して候補者 DB に照合
python3 tools/mac/line_import.py ~/Downloads/export.txt --dry-run
# 3) 取り込み（同じ候補者ページに追記）
python3 tools/mac/line_import.py ~/Downloads/export.txt --self "佐藤 雄太"
```

- **候補者の特定**：ヘッダ「〇〇とのトーク履歴」の相手名を候補者 DB の `名前` に照合。一致しなければ
  `--candidate "氏名"` か `--candidate-id <page_id>` で明示する。
- **送受信判定**：自分の LINE 表示名（`--self` または `LINE_SELF_NAME`）と一致する送信者を「送信」に。
  未指定でも 1:1 なら「相手以外の送信者」を自分と推定するが、確実にするなら `--self` を渡す。
- **重複防止**：取り込んだメッセージの署名（日時＋送信者＋本文）を `LINE_WATERMARK_FILE` に記録し、
  前回と重なる範囲を含む `.txt` を渡しても二重追記しない。
- **制約**：全自動ではない（都度の手動書き出しが要る）。LINE は「表示済みの分だけ」書き出す。
